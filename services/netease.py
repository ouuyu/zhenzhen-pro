...
class NeteaseMusicPlayer:
    ...
    def get_music_player_html(self, search_keywords: str) -> str:
        try:
            ...
            if music_response.status_code != 200 or not (music_data.get("url") and music_data.get("status") == 200):
                music_response.raise_for_status()
            ...
            # 压缩 HTML：去除注释、标签间空白和多余空格
            html_output = re.sub(r"<!--.*?-->", "", html_output, flags=re.DOTALL)  # 删除 HTML 注释
            html_output = re.sub(r">\s+<", "><", html_output)  # 删除标签之间的空白
            html_output = re.sub(r"\s+", " ", html_output)    # 将多个空格合并为一个（可选）
            html_output = re.sub(r" +(<)", r"\1", html_output)  # 删除标签前多余的空格
            html_output = re.sub(r"(>)\s+", r"\1", html_output)  # 删除标签后多余的空格
